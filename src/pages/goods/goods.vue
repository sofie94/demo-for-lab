...
import { APIgetGoodsById } from '@/services/goods'
...
import { APIpostMemberCart } from '@/services/cart'
...
const getGoodsByIdData = async () => {
  const res = await APIgetGoodsById(props.id)
  ...
  localdata.value = {
    ...
    sku_list: res.result.skus.map((item) => {
      return {
        ...
        image: item.picture,
        price: item.price * 100, // 注意：需要乘以 100
...
const onAddCart = async (e: SkuPopupEvent) => {
  await APIpostMemberCart({ skuId: e._id, count: e.buy_num })
...
<template>
  ...
  <scroll-view scroll-y class="viewport">
    ...
    <view class="goods">
      <!-- 商品主图 -->
      <view class="preview">
        <swiper circular autoplay :interval="1000" @change="onChange">
