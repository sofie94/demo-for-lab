...
import { APIgetMebmerOrder, APIgetMebmerOrderNow, APIpostMemberOrder } from '@/services/order'
...
const getMebmerOrderData = async () => {
  if (props.count && props.skuId) {
    const res = await APIgetMebmerOrderNow({
      count: props.count,
      skuId: props.skuId,
    })
    orderPre.value = res.result
  } else {
    const res = await APIgetMebmerOrder()
    orderPre.value = res.result
  }
}
...
const onOrderSubmit = async () => {
  ...
  const res = await APIpostMemberOrder({
...
<template>
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    ...
    <view class="button" :class="{ disabled: !selectAddress?.id }" @click="onOrderSubmit">
      提交订单
    </view>
  </view>
</template>
