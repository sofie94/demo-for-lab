...
import { ref } from 'vue'
import { APIgetMemberOrderById, deleteMemberOrderAPI } from '@/services/order'
import type { OrderResult, LogisticItem } from '@/types/order'
import { OrderState, orderStateList } from '@/types/constants'
import { APIgetPayWxPayMiniPay, APIgetPayMock } from '@/services/pay'
import {
...
const getMemberOrderByIdData = async () => {
  const res = await APIgetMemberOrderById(query.id)
...
const onOrderPay = async () => {
  ...
  if (import.meta.env.DEV) {
    ...
    await APIgetPayMock({ orderId: query.id })
  } else {
    // 生产环境：获取支付参数 + 发起微信支付
    const res = await APIgetPayWxPayMiniPay({ orderId: query.id })
